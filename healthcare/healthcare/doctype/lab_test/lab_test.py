...
class LabTest(Document):
	...
	def check_is_external_lab(self):
		if not self.observation_request:
			frappe.msgprint("No Observation request in lab test")
			return
		request_doc = frappe.get_doc("Observation Request", self.observation_request)
		# frappe.msgprint(str(request_doc.name))
		if not request_doc.external_labs: return
		for lab in request_doc.external_labs:
			if lab.observation == self.lab_test_name:
				self.is_external_lab = 1
				return
		self.is_external_lab = 0
...
def create_test_from_template(lab_test):
	...
	lab_test.custom_result = template.custom_result
...
def create_compounds(template, lab_test, is_group):
	...
	for normal_test_template in template.normal_test_templates:
		...
		normal.template = template.name
		normal.event_group = normal_test_template.event_group
