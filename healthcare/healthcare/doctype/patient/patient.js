...
frappe.ui.form.on('Patient', {
	refresh: function (frm) {
		frm.add_custom_button(__('Patient Encounter'), function() {
			redirectToPatientEncounter(frm); });
		
		function redirectToPatientEncounter(frm) {
			// Gather required data from Patient doctype
			var patient_name = frm.doc.patient_name;
			var date_of_birth = frm.doc.date_of_birth;
			var uid = frm.doc.uid; 
			// Add more fields as required
		
			// Redirect to Patient Encounter with data
			frappe.new_doc('Patient Encounter', {
				'patient': patient_name,
				'date_of_birth': date_of_birth,
				'uid': uid
				// Pass more fields as required
			});
		}
