...
class Patient(Document):
	...
	def before_save(self):
		...
		self.birth_date = frappe.utils.data.format_date(self.dob, "dd/mm/yyyy")

	def validate(self):
		# self.validate_duplicate_patient()
	...
	def before_insert(self):
		...
		self.patient_code = get_new_patient_code()
	...
	def validate_duplicate_patient(self):
		exists=frappe.db.sql(f"select count(name) from `tabPatient`  where (patient_name='{self.patient_name}' and dob='{self.dob}' ) or (uid='{self.uid}')",as_list=1)[0][0]
		if exists > 0:
			frappe.throw(str("Not a duplicate patient "))
...
def get_new_patient_code():
	prefix = "PC-"
	setting = frappe.get_doc("Healthcare Settings")
	code = setting.last_patient_code
	code += 1
	setting.last_patient_code = code
