...
class InpatientRecord(Document):
	...
	def validate_dulicate(self):
		...
		records = frappe.get_all('Inpatient Record', 
								filters={'modified': ['>=', time_threshold],'name':self.name},
								fields=['*'])
	...
	def validate_already_scheduled_or_admitted(self):
		query = """
			...
			and name != %(name)s and patient = %(patient)s and reservation_type=%(reservation_type)s
			"""

		ip_record = frappe.db.sql(query, {"name": self.name, "patient": self.patient,"reservation_type":self.reservation_type}, as_dict=1)
