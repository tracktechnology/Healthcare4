...
class MedicalServiceTemplate(Document):
...
def change_item_code_from_template(item_code, doc):
	...
	if frappe.db.exists("Item", {"item_code": item_code}):
		...
		frappe.db.set_value("Medical Service Template", doc.name, "item_code", item_code)
	return
...
def update_item_and_item_price(doc):
	if doc.is_billable and doc.item:
		...
		item_doc.description = (
			doc.description if doc.get("doctype") == "Medical Service Template" else ""
		)
		item_doc.disabled = 0
